...
const start = async () => {
    try {
        const btcProcess = fork(join(__dirname, `./btc/index.ts`), [], { execArgv: ['-r', 'ts-node/register'] })
        const trxProcess = fork(join(__dirname, `./trx/index.ts`), [], { execArgv: ['-r', 'ts-node/register'] })
